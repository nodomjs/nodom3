...
var Renderer = /** @class */ (function () {
    ...
    Renderer.render = function () {
        for (; this.waitList.length > 0;) {
            modulefactory_1.ModuleFactory.get(this.waitList.shift()).render();
        }
    };
    /**
     * 渲染dom
     ...
     * @returns
     */
    Renderer.renderDom = function (module, src, model, parent, key) {
        var dst = {
        ...
        if (dst.tagName) {
            if (!dst.notChange) {
                ...
                if (!cssmanager_1.CssManager.handleStyleDom(module, src, Renderer.currentModuleRoot, src.getProp('scope') === 'this')) {
                    //assets
                    if (src.assets && src.assets.size > 0) {
                        for (var _i = 0, _a = src.assets; _i < _a.length; _i++) {
                            var p = _a[_i];
                            dst[p[0]] = p[1];
                        }
                    }
            ...
            if (src.events && !module.eventFactory.getEvent(dst.key)) {
                for (var _b = 0, _c = src.events; _b < _c.length; _b++) {
                    var evt = _c[_b];
            ...
            if (src.children && src.children.length > 0) {
                dst.children = [];
                for (var _d = 0, _e = src.children; _d < _e.length; _d++) {
                    var c = _e[_d];
            ...
            if (src.expressions) { //文本节点
                var value_1 = '';
                src.expressions.forEach(function (v) {
                    if (v instanceof expression_1.Expression) { //处理表达式
                        var v1 = v.val(module, dst.model);
                        value_1 += v1 !== undefined ? v1 : '';
                    }
                    else {
                        value_1 += v;
                    }
                });
                dst.textContent = value_1;
                dst.staticNum = -1;
            }
            else {
                dst.textContent = src.textContent;
        ...
        function handleDirectives() {
            ...
            dst.staticNum = -1;
            for (var _i = 0, _a = src.directives; _i < _a.length; _i++) {
                var d = _a[_i];
                //model指令不执行
        ...
        function handleProps() {
            ...
            var classReg = /^class$/i;
            var value;
            for (var _i = 0, _a = src.props; _i < _a.length; _i++) {
                var k = _a[_i];
                if (Array.isArray(k[1])) { //数组，需要合并
                    value = [];
                    for (var i = 0; i < k[1].length; i++) {
                        var a = k[1][i];
                        if (a instanceof expression_1.Expression) {
                ...
                else if (k[1] instanceof expression_1.Expression) {
     ...
     */
    Renderer.renderToHtml = function (module, src, parentEl, isRenderChild) {
        var el = module.getNode(src.key);
        if (el) { //html dom节点已存在
            if (src.tagName) {
                ...
                //设置属性
                for (var _i = 0, _a = Object.keys(src.props); _i < _a.length; _i++) {
                    var p = _a[_i];
        ...
        function newEl(dom) {
            ...
            //创建element
            var el = document.createElement(dom.tagName);
            //保存虚拟dom
            el['vdom'] = dom.key;
            ...
            if (!dom.subModuleId) {
                ...
                //asset
                if (dom.assets) {
                    for (var _b = 0, _c = Object.keys(dom.assets); _b < _c.length; _b++) {
                        var p = _c[_b];
                        el[p] = dom.assets[p];
                    }
                }
                //处理event
                eventmanager_1.EventManager.bind(module, dom);
            }
            return el;
        }
        /**
        ...
        function newText(dom) {
            ...
            if (cssmanager_1.CssManager.handleStyleTextDom(module, dom)) {
                return;
            }
            var node = document.createTextNode(dom.textContent || '');
        ...
        function genSub(pEl, vdom) {
            if (vdom.children && vdom.children.length > 0) {
                vdom.children.forEach(function (item) {
                    var el1;
                    if (item.tagName) {
                        el1 = newEl(item);
                        genSub(el1, item);
        ...
        function handleAssets(dom, el) {
            //处理asset
            if (dom.assets) {
                for (var _i = 0, _a = Object.keys(dom.assets); _i < _a.length; _i++) {
                    var k = _a[_i];
                    el[k] = dom.assets[k];
                }
            }
     ...
     */
    Renderer.handleChangedDoms = function (module, changeDoms) {
        for (var _i = 0, changeDoms_1 = changeDoms; _i < changeDoms_1.length; _i++) {
            var item = changeDoms_1[_i];
            var _a = [
