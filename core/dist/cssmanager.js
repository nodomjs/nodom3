...
var CssManager = /** @class */ (function () {
     ...
     */
    CssManager.handleStyleDom = function (module, dom, root, add) {
        if (dom.tagName.toLowerCase() !== 'style') {
            return false;
        }
        if (add) {
            var cls = this.cssPreName + module.id;
            if (root.props['class']) {
     ...
     */
    CssManager.handleStyleTextDom = function (module, dom) {
     ...
     */
    CssManager.addRules = function (module, cssText, scopeName) {
        //sheet 初始化
        if (!this.sheet) {
        ...
        var reg = /(@[a-zA-Z]+\s+url\(.+?\))|([.#@a-zA-Z]\S*(\s*\S*\s*?)?{)|\}/g;
        //import support url正则式
        var regImp = /@[a-zA-Z]+\s+url/;
        ...
        while ((re = reg.exec(cssText)) !== null) {
            if (regImp.test(re[0])) { //import namespace
                ...
                if (startIndex >= 0 && --beginNum <= 0) { //style @ end
                    var txt = cssText.substring(startIndex, re.index + 1);
                    if (txt[0] === '@') { //@开头
        ...
        function handleStyle(module, cssText, scopeName) {
            var reg = /.+(?=\{)/; //匹配字符"{"前出现的所有字符
            var r = reg.exec(cssText);
            if (!r) {
                return;
            }
            // 保存样式名，在模块 object manager 中以数组存储
            if (scopeName) {
                var arr = module.objectManager.get('$cssRules');
                if (!arr) {
                    arr = [];
        ...
        function handleImport(cssText) {
            var ind = cssText.indexOf('(');
            var ind1 = cssText.lastIndexOf(')');
            if (ind === -1 || ind1 === -1 || ind >= ind1) {
                return;
            }
            var css = cssText.substring(ind, ind1);
     ...
     */
    CssManager.clearModuleRules = function (module) {
        var rules = module.objectManager.get('$cssRules');
